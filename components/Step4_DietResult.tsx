...
import { UserData, DietPlan, MacroTargets, DietMeal, Meal, AiMealConfig } from '../types';
...
const loadingMessages = [
    ...
    "Consultando o banco de dados de alimentos...",
    "Selecionando as melhores opções para você...",
    "Montando seu plano de refeições...",
...
const LoadingIndicator: React.FC<{isRecalculating?: boolean}> = ({isRecalculating = false}) => {
    ...
    const title = isRecalculating ? "A IA está recalculando..." : "Gerando seu plano...";
    const subtitle = isRecalculating 
        ? "Estamos ajustando o restante da sua dieta para incorporar suas edições e manter suas metas. Só um momento."
        : "Estamos montando o plano de dieta perfeito para você. Isso pode levar um momento.";
        ...
        <div className="text-center p-8">
            ...
            <div className="space-y-3 text-left">
                {loadingMessages.map((msg, index) => (
                    <div key={index} className="flex items-center gap-3 transition-opacity duration-500" style={{ opacity: index <= messageIndex ? 1 : 0.3 }}>
                         <div className={`w-5 h-5 rounded-full flex items-center justify-center ${index < messageIndex ? 'bg-green-500' : 'bg-slate-600'}`}>
                            {index < messageIndex && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                        </div>
                        <span className={`transition-colors ${index <= messageIndex ? 'text-slate-200' : 'text-slate-500'}`}>{msg}</span>
                    </div>
                ))}
            </div>
        </div>
...
const Step4DietResult: React.FC<Props> = ({
  ...
  const clinicalAnalysisRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const fetchDietPlan = async () => {
      ...
      setError(null);
      try {
        const plan = await generateDietPlan(userData, targetCalories, macroTargets, goal, plannerConfig, bloodTestFile);
  ...
  const handleRecalculateAfterEdit = async (updatedMeal: DietMeal) => {
    ...
    try {
        const newPlan = await generateDietPlan(
            ...
            plannerConfig,
            bloodTestFile,
            mealsForRecalculation
        );

        const fullNewPlan: DietPlan = {
            ...newPlan,
            userData,
            originalMeals: plannerConfig.data,
            macroTargets,
  ...
  const handleGeneratePdf = () => {
    ...
    const analysisElement = clinicalAnalysisRef.current;

    if (input) {
        setIsGeneratingPdf(true);
        
        // Hide analysis section if not included
        const originalDisplayStyle = analysisElement ? analysisElement.style.display : '';
        if (analysisElement && !includeAnalysisInPdf) {
        ...
        html2canvas(input, {
            ...
            alert("Ocorreu um erro ao gerar o PDF. Por favor, tente novamente.");
             ...
             // Restore analysis section display
            if (analysisElement) {
                analysisElement.style.display = originalDisplayStyle;
            }
  ...
  return (
    <div className="bg-slate-800 p-4 sm:p-8 rounded-2xl shadow-lg border border-slate-700 animate-fade-in relative">
        ...
        <div ref={dietPlanRef} className="p-4 bg-slate-800">
            ...
            {displayedPlan.clinical_analysis && displayedPlan.clinical_analysis.observacoes.length > 0 && (
                <div ref={clinicalAnalysisRef} className="mt-8">
                    <button onClick={() => setIsAnalysisOpen(!isAnalysisOpen)} className="w-full flex justify-between items-center p-4 bg-slate-900/50 rounded-xl border border-slate-700">
                        <h3 className="text-xl font-semibold text-white flex items-center gap-3">
                            <Stethoscope className="w-6 h-6 text-cyan-400"/>
                            Observações da Análise Clínica
                        </h3>
                        <ChevronDown className={`w-6 h-6 text-slate-400 transition-transform ${isAnalysisOpen ? 'rotate-180' : ''}`} />
                    </button>
                    {isAnalysisOpen && (
                        <div className="p-6 bg-slate-900/50 rounded-b-xl border border-t-0 border-slate-700 animate-fade-in-up">
                            <div className="space-y-3 text-slate-300 mb-6">
                            {displayedPlan.clinical_analysis.observacoes.map((obs, index) => (
                                <p key={index} className="pl-4 border-l-2 border-cyan-500">{obs}</p>
                            ))}
                            </div>

                            {displayedPlan.clinical_analysis.suplementos.length > 0 && (
                                <div className="border-t border-slate-700 pt-4">
                                    <h4 className="text-lg font-semibold text-cyan-300 mb-3">Sugestões de Fitoterápicos</h4>
                                    <div className="space-y-4">
                                        {displayedPlan.clinical_analysis.suplementos.map((sup, index) => (
                                            <div key={index} className="p-4 bg-slate-800 rounded-lg border border-slate-600">
                                                <p className="font-bold text-white">{sup.nome}</p>
                                                <p className="text-sm text-slate-400"><strong>Objetivo:</strong> {sup.objetivo}</p>
                                                <p className="text-sm text-slate-400"><strong>Dose Sugerida:</strong> {sup.dose_sugerida ? `${sup.dose_sugerida} ${sup.unidade}` : 'A definir clinicamente'}</p>
                                                <p className="text-xs text-slate-500 mt-2"><em><strong>Evidência:</strong> {sup.resumo_evidencia} (Ref: {sup.citacao})</em></p>
                                                <p className="mt-2 p-2 text-xs bg-yellow-500/10 text-yellow-300 border border-yellow-500/30 rounded"><strong>Atenção:</strong> {sup.seguranca}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {displayedPlan.clinical_analysis.metadados.requer_validacao_clinica && (
                                <p className="text-xs text-slate-500 mt-6 text-center">
                                    Todas as sugestões baseadas em exames laboratoriais são de caráter informativo e requerem validação por um profissional de saúde.
                                </p>
                            )}
                        </div>
                    )}
            ...
            <div className="mt-8 p-6 bg-slate-900/50 rounded-xl border border-slate-700">
                ...
                <div className="space-y-4">
                    ...
                    {displayedPlan.recommendations?.map((rec, index) => {
                       const icon = /água|hidratação/i.test(rec) ? <Droplets className="w-5 h-5 text-blue-400"/> : <Info className="w-5 h-5 text-green-400"/>;
                       const color = /água|hidratação/i.test(rec) ? "bg-blue-500/20" : "bg-green-500/20";
                       return (
                            <div className="flex items-start gap-4" key={index}>
                                <div className={`p-2 ${color} rounded-full mt-1`}>{icon}</div>
                                <div>
        ...
        <div className="mt-8 p-6 bg-slate-900/50 rounded-xl border border-slate-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-center">
                <button 
                    ...
                    disabled={isSaved}
                    className="flex flex-col items-center justify-center p-4 bg-slate-800 rounded-lg hover:bg-slate-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    <Save className={`w-6 h-6 mb-2 ${isSaved ? 'text-green-400' : 'text-cyan-400'}`} />
                    <span className="text-sm font-semibold text-slate-300">{isSaved ? 'Salvo!' : 'Salvar Dieta'}</span>
                </button>
                 <button onClick={handleGeneratePdf} disabled={isGeneratingPdf} className="flex flex-col items-center justify-center p-4 bg-slate-800 rounded-lg hover:bg-slate-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <span className="text-sm font-semibold text-slate-300">{isGeneratingPdf ? 'Gerando...' : 'Baixar PDF'}</span>
                </button>
            ...
            {displayedPlan.clinical_analysis && (
                 <div className="mt-4">
                    <label className="flex items-center justify-center text-sm text-slate-400">
                        <input 
                            type="checkbox"
                            checked={includeAnalysisInPdf}
                            onChange={(e) => setIncludeAnalysisInPdf(e.target.checked)}
                            className="w-4 h-4 mr-2 accent-cyan-500 bg-slate-600 rounded"
                        />
                        Incluir análise clínica no PDF
                    </label>
                </div>
            )}
        </div>
